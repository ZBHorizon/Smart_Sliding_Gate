...
#include <iostream>
 ...
 bool INA226::initialize() {
     ...
     i2c_fd = wiringPiI2CSetup(INA226_I2C_ADDRESS);
     if (i2c_fd < 0) {
     ...
     if (wiringPiI2CWriteReg16(i2c_fd, CONFIG_REGISTER, CONFIG_VALUE) < 0) {
         std::cerr << "write configuration register failed!" << std::endl;
         return false;
     }
 
     // Write the calibration register.
     if (wiringPiI2CWriteReg16(i2c_fd, CALIBRATION_REGISTER, CALIBRATION_VALUE) < 0) {
 ...
 // Static class member for the I2C file descriptor.
 int INA226::i2c_fd = -1;
  ...
  * and multiplies by CURRENT_LSB to get mA. If the read fails,
 ...
 float INA226::readCurrent_mA() {
     ...
     int raw = wiringPiI2CReadReg16(i2c_fd, CURRENT_REGISTER);
     if (raw < 0) {
     ...
     // Convert to milliamps: Current = raw_current * CURRENT_LSB.
     float current_mA = static_cast<float>(raw_current) * CURRENT_LSB;
     return current_mA;
 }
 ...
 bool INA226::power_down()
 {
    ...
    int raw_config = wiringPiI2CReadReg16(i2c_fd, CONFIG_REGISTER);
    if (raw_config < 0) {
        return false;
    }
    ...
    if (wiringPiI2CWriteReg16(i2c_fd, CONFIG_REGISTER, write_val) < 0) {
        return false;
    }
    return true;
 }
