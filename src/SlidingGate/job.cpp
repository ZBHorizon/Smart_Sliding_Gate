...
bool job::create_job(float target_position) {
  ...
  keyframe start_keyframe { .speed = Motor::read_speed(), .timePoint = steady_clock::now() };
  ...
  LOG_INFO() << "\033[35mTarget Time: " << std::chrono::duration_cast<std::chrono::milliseconds>(target_timePoint - start_keyframe.timePoint).count()
             << " ms\033[0m";
  ...
  if (std::abs(start_keyframe.speed) > 0.0f) {
    ...
    if (std::signbit(start_keyframe.speed) == std::signbit(-target_direction)) { _keyframes.push_back(compute_braking_keyframe()); }
  }

  // If current speed is below max, add an acceleration keyframe.
  if (std::abs(_keyframes.back().speed) < _MAX_SPEED) { _keyframes.push_back(compute_acceleration_keyframe(target_timePoint, target_direction)); }

  // Deceleration phase: add a keyframe _RAMP_TIME_ms before the target time.
  if (!skip_deceleration_phase) { _keyframes.push_back(compute_deceleration_keyframe(target_timePoint, target_direction)); }
...
float job::get_speed() {
  ...
  if (_fully_open
      && std::prev(_keyframes.end())->timePoint - std::chrono::milliseconds(static_cast<int>(_RAMP_TIME_ms.count() * 0.1)) < current_timePoint) {
    // If the current iterator's position is before the request, return minimal open speed.
    LOG_INFO() << "waiting for " << "\033[35m" << "open endswitch" << "\033[0m" << " with speed " << "\033[35m" << _MIN_SPEED * 100.0f
               << " % .\033[0m";
    return _MIN_SPEED;
  }
  if (_fully_close
    ...
    LOG_INFO() << "waiting for " << "\033[35m" << "close endswitch" << "\033[0m" << " with speed " << "\033[35m" << -_MIN_SPEED * 100.0f
               << " % .\033[0m";
    return -_MIN_SPEED;
  }
  ...
  for (; next_iter != _keyframes.end(); ++_current_iter, ++next_iter) {
    ...
    float interpolated_speed = _current_iter->speed
                               + ((next_iter->speed - _current_iter->speed) * (current_timePoint - _current_iter->timePoint)
  ...
  if (std::chrono::duration<float, std::milli>(std::prev(_keyframes.end())->timePoint.time_since_epoch()) * 1.1f
      < std::chrono::duration<float, std::milli>(current_timePoint.time_since_epoch())) {
...
job::keyframe job::compute_acceleration_keyframe(steady_clock::time_point target_time, float target_direction) {
  ...
  if (accel_duration + _RAMP_TIME_ms > available_time) {
    ...
    newSpeed                               = _keyframes.back().speed
               + ((target_direction * _MAX_SPEED) - _keyframes.back().speed)
...
void job::print_keyframes() {
  ...
  LOG_INFO() << "\033[35mKeyframe: " << keyframeIndex++ << ", Time: " << 0 << " ms, Speed: " << iter->speed * 100.0f << " % \033[0m";
  ++iter;
  ...
  for (; iter != _keyframes.end(); ++iter) {
    ...
    LOG_INFO() << "\033[35mKeyframe: " << keyframeIndex++ << ", Time: " << delta << " ms, Speed: " << iter->speed * 100.0f << " % \033[0m";
  }
  LOG_INFO() << LOG_LINE_50;
}
