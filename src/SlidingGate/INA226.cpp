 ...
 */
inline static std::int16_t swap_bytes(std::int16_t value) { return static_cast<std::int16_t>(((value & 0xFF) << 8) | ((value >> 8) & 0xFF)); }
...
bool INA226::power_down() {
  ...
  config &= ~0b00000000'00000111; // zero out bits 0..2
...
bool INA226::wake_up() {
  ...
  config &= ~0b00000000'00000111; // clear bits [2..0]
  config |= 0b00000000'00000111;  // set them to 111
