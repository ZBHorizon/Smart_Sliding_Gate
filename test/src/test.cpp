...
void IO::pinMode(int pin, int mode) {
  ...
  LOG_INFO() << "pinMode: Pin " << Test_IO::ORANGE << pin << Test_IO::RESET << " (" << Test_IO::ORANGE << Test_IO::getPinName(pin) << Test_IO::RESET
...
void IO::pullUpDnControl(int pin, int pud) {
  ...
  LOG_INFO() << "pullUpDnControl: Pin " << Test_IO::ORANGE << pin << Test_IO::RESET << " (" << Test_IO::ORANGE << Test_IO::getPinName(pin)
             << Test_IO::RESET << ") set to pull " << Test_IO::ORANGE << Test_IO::pullUpToString(pud) << Test_IO::RESET << ".";
}
...
int IO::digitalRead(int pin) {
  ...
  LOG_INFO() << "digitalRead: Pin " << Test_IO::ORANGE << pin << Test_IO::RESET << " (" << Test_IO::ORANGE << Test_IO::getPinName(pin)
             << Test_IO::RESET << ") reads " << Test_IO::ORANGE << Test_IO::digitalValToString(state) << Test_IO::RESET << ".";
  return state;
}
...
void IO::digitalWrite(int pin, int value) {
  ...
  if (value != LOW && value != HIGH) {
    LOG_ERROR() << "Error: Invalid digital value for pin " << Test_IO::ORANGE << pin << Test_IO::RESET << " (" << Test_IO::ORANGE
                << Test_IO::getPinName(pin) << Test_IO::RESET << "), only LOW or HIGH allowed.";
  }
  ...
  LOG_INFO() << "digitalWrite: Pin " << Test_IO::ORANGE << pin << Test_IO::RESET << " (" << Test_IO::ORANGE << Test_IO::getPinName(pin)
             << Test_IO::RESET << ") set to " << Test_IO::ORANGE << Test_IO::digitalValToString(value) << Test_IO::RESET << ".";
  if (MainWindow::s_instance) MainWindow::s_instance->updateTable(pin);
}
...
void IO::pwmWrite(int pin, int value) {
  ...
  if (Test_IO::global_PWM::PWM_MODE == -1) { LOG_ERROR() << "Error: PWM mode not set."; }
  if (Test_IO::global_PWM::PWM_CLOCK == -1) { LOG_ERROR() << "Error: PWM clock not set."; }
  if (Test_IO::global_PWM::PWM_RANGE == -1) { LOG_ERROR() << "Error: PWM range not set."; }
  if (value > Test_IO::global_PWM::PWM_RANGE || value < 0) {
    LOG_ERROR() << "Error: pwmWrite value for pin " << Test_IO::ORANGE << pin << Test_IO::RESET << " (" << Test_IO::ORANGE << Test_IO::getPinName(pin)
                << Test_IO::RESET << ") exceeds the set PWM range of " << Test_IO::ORANGE << Test_IO::global_PWM::PWM_RANGE << Test_IO::RESET << ".";
  }
  ...
  LOG_INFO() << "pwmWrite: Pin " << Test_IO::ORANGE << pin << Test_IO::RESET << " (" << Test_IO::ORANGE << Test_IO::getPinName(pin) << Test_IO::RESET
             << ") PWM value " << Test_IO::ORANGE << value << Test_IO::RESET << " (" << Test_IO::ORANGE << converted_value * 100.0f << "%"
             << Test_IO::RESET << ") set.";
  if (MainWindow::s_instance) MainWindow::s_instance->updateTable(pin);
}
...
void IO::pwmSetRange(std::uint32_t range) {
  Test_IO::check_wiringPiSetup();
  if (range == 0) { LOG_ERROR() << "Error: PWM range cannot be zero."; }
...
void IO::pwmSetClock(int divisor) {
  Test_IO::check_wiringPiSetup();
  if (divisor <= 0) { LOG_ERROR() << "Error: PWM clock divisor must be positive."; }
...
int IO::wiringPiISR(int pin, int mode, void (*function)(void)) {
  ...
  LOG_INFO() << "wiringPiISR: ISR for pin " << Test_IO::ORANGE << pin << Test_IO::RESET << " (" << Test_IO::ORANGE << Test_IO::getPinName(pin)
             << Test_IO::RESET << ") with mode " << Test_IO::ORANGE << Test_IO::isrModeToString(mode) << Test_IO::RESET << " and function "
             << Test_IO::ORANGE << Test_IO::isrfuntionNameToString(function) << Test_IO::RESET << " registered.";
  return 0;
}
...
void IO::delayMicroseconds(std::uint32_t howLong) {
  ...
  // }
  // LOG_INFO() << "delayMicroseconds: Waiting " << howLong << " us." ;
...
void Test_IO::isr_sim_update(int pin) {
  ...
  if (Test_IO::pin_states[pin].current_signal == Test_IO::pin_states[pin].previous_signal) { return; }
  if (Test_IO::pin_states[pin].isr_mode == INT_EDGE_RISING
      && !(Test_IO::pin_states[pin].current_signal == 1.0f && Test_IO::pin_states[pin].previous_signal == 0.0f)) {
    return;
  }
  if (Test_IO::pin_states[pin].isr_mode == INT_EDGE_FALLING
      && !(Test_IO::pin_states[pin].current_signal == 1.0f && Test_IO::pin_states[pin].previous_signal == 0.0f)) {
    return;
  }
  ...
  LOG_INFO() << "ISR triggered on pin " << Test_IO::ORANGE << pin << Test_IO::RESET << " (" << Test_IO::ORANGE << Test_IO::getPinName(pin)
             << Test_IO::RESET << ") with mode " << Test_IO::ORANGE << isrModeToString(Test_IO::pin_states[pin].isr_mode) << Test_IO::RESET << ".";
}
...
float Test_IO::read_pin(int pin) { return Test_IO::pin_states[pin].current_signal; }
...
void Test_IO::check_wiringPiSetup() {
  if (!wiringpi_setup_called) { LOG_ERROR() << "Error: wiringPiSetup must be called before."; }
}
